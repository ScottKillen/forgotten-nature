...
public class BlockHempCotBamCorn extends BlockFlower
{
    ...
    {
        return par1 == Block.tilledField.blockID || par1 == this.blockID || par1 == Block.dirt.blockID || par1 == Block.grass.blockID;
    }
    ...
    public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if (par1World.getBlockLightValue(par2, par3 + 1, par4) >= 9)
        {
            ...
            if((var6 == 0 || var6 == 4 || var6 == 12) && par1World.getBlockId(par2, par3 - 1, par4) == this.blockID)
            {
            	par1World.setBlock(par2, par3, par4, 0, 0, 2);
            }
            
            if(var6 > 8 && var6 < 12 && par1World.getBlockMetadata(par2, par3 - 1, par4) == var6 - 1 && par1World.getBlockId(par2, par3 - 1, par4) == this.blockID)
            {
            	par1World.setBlock(par2, par3, par4, 0, 0, 2);
            }
            
            if(par1World.getBlockId(par2, par3 - 9, par4) != this.blockID)
            {
            	if(var6 == 11 && forgottennature.bambooSpread)
            	{
            		for(int spreadx = -1; spreadx < 2; spreadx++)
            		{
            			...
            			{
            				if(par5Random.nextInt(forgottennature.bamSpreadBuffer) == 0 && (par1World.getBlockId(par2 + spreadx, par3 - 2, par4 + spreadz) == Block.dirt.blockID || par1World.getBlockId(par2 + spreadx, par3 - 2, par4 + spreadz) == Block.grass.blockID) && par1World.getBlockId(par2 + spreadx, par3 - 1, par4 + spreadz) == 0)
            				{
            					par1World.setBlock(par2 + spreadx, par3 - 1, par4 + spreadz, this.blockID, 8, 2);
            				}
            			}
            		}
            	}
	            ...
	            {
	            	par1World.setBlock(par2, par3 + 1, par4, this.blockID, 8, 2);
	            }
            }
            ...
            int underblock = par1World.getBlockId(par2, par3 - 1, par4);
            int undermet = par1World.getBlockMetadata(par2, par3 - 1, par4);
            if((forgottennature.bambooGeneration && var6 > 7 && var6 < 11 && par5Random.nextInt(2) == 0) && (underblock == Block.dirt.blockID || underblock == Block.grass.blockID || (underblock == this.blockID && (undermet == var6 + 2 || undermet == 11))))
            {
            	par1World.setBlock(par2, par3, par4, this.blockID, var6 + 1, 2);
            }
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int par2, float par3, float par4, float par5)
    {
        ...
        if ((equipped.getItem() == Item.dyePowder) && (equipped.getItemDamage() == 15))  //if bone meal
        {
        	...
        	if(full == 1 && world.getBlockMetadata(i, j + 1, k) == 0)
        		world.setBlock(i, j + 1, k, this.blockID, 3, 2);
    ...
    public void dropBlockAsItemWithChance(World world, int i, int j, int k, int met, float par6, int par7)
    {
        ...
        if (met == 3)
        {
        	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[foodID],1,3));//corn
        }
        else if (met == 7)
        {
        	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[forgottennature.newCropGoodsID],1,0));//cotton
        }
        else if (met == 11)
        {
        	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[forgottennature.hempCottonBambooCornID],1,11));//bamboo
        }
        else if (met == 15)
        {
        	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[forgottennature.newCropGoodsID],1,1));//hemp
        }
        ...
        for (int n = 0; n < 5; n++)
        {
            ...
            {
            	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[seedID],1,1));
            }
            else if ((met == 7) && (world.rand.nextInt(3) == 0))
            {
            	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[seedID],1,2));
            }
            else if ((met == 15) && (world.rand.nextInt(3) == 0))
            {
            	dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[seedID],1,8));
            }
        }
    }
