...
public class BlockNetherLeaves extends BlockLeavesBase implements IShearable
{
    ...
    public Icon getIcon(int par1, int par2)
    {
    	...
    	if(fan == 0)
    		...
    		return this.field_94349_a[par2+2];
    }
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        if (!world.isRemote)
        {
	        ...
	        if (((l & 8)!= 0) && leafDecay == true && random.nextInt(newLeafDecayRate) == 0)
	        {
	        	byte var7 = 4;
                int var8 = var7 + 1;
                byte var9 = 32;
    ...
    public void dropBlockAsItemWithChance(World world, int i, int j, int k, int l, float f, int i1)
    {
        ...
        if (world.rand.nextInt(14) == 0)
        {
            if (l == 0)
            {
            	dropBlockAsItem_do(world, i, j, k, new ItemStack(forgottennature.newSap3, 1, 3));    //nether ash
            }
            if (l == 1)
            {
            	dropBlockAsItem_do(world, i, j, k, new ItemStack(forgottennature.newSap3, 1, 4));    //nether blaze
            }
        }
    }
    /**
