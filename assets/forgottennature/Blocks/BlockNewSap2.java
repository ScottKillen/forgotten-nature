...
public class BlockNewSap2 extends BlockFlower
{
    ...
    public void registerIcons(IconRegister par1IconRegister)
    {
		...
		//this.field_94349_a[7] = par1IconRegister.registerIcon("forgottennature:SaplingCocoa");
    ...
    public void growTree(World world, int i, int j, int k, Random random)
    {
        ...
        {
            obj = new genPoplar(forgottennature.leafIDindex+2, 7, 17, 1);    //poplar
        }
        else if (l == 5)
        {
            obj = new genBeech(forgottennature.leafIDindex+3, 0, forgottennature.logIDindex+1, 6);    //beech
        }
        else if (l == 6)
        {
            obj = new genWalnut(forgottennature.leafIDindex+3, 1, forgottennature.logIDindex+1, 8);    //walnut
        }
//        else if (l == 7)
//        {
//            obj = new genCocoa(forgottennature.leafIDindex+3, 2, forgottennature.logIDindex+1, 10);    //cocoa
//        }
        else if (l == 8)
        {
            obj = new genWTEucalyptus(forgottennature.leafIDindex+3, 3, forgottennature.logIDindex+1, 14);    //WTEucal
        }
        else if (l == 9)
        {
            obj = new genBukkit(forgottennature.leafIDindex+3, 4, forgottennature.logIDindex+2, 0);    //bukkit
        }
        else if (l == 10)
        {
            obj = new genPalm(forgottennature.leafIDindex+3, 5, forgottennature.logIDindex+2, 1);    //banana
        }
        else if (l == 11)
        {
            obj = new genFruit(false, forgottennature.leafIDindex+3, 6, forgottennature.logIDindex+2, 2, 1);    //orange
        }
        else if (l == 12)
        {
            obj = new genFruit(false, forgottennature.leafIDindex+3, 7, forgottennature.logIDindex+2, 3, 1);    //peach
        }
        else if (l == 13)
        {
            obj = new genFruit(false, forgottennature.leafIDindex+4, 0, forgottennature.logIDindex+2, 4, 1);    //lemon
        }
        else if (l == 14)
        {
            obj = new genFruitBush(forgottennature.leafIDindex+4, 1, 0);    //blueberry
        }
        else if (l == 15)
        {
            obj = new genFruitBush(forgottennature.leafIDindex+4, 3, 0);    //raspberry
        }
        else
        {
            obj = new genPalm(forgottennature.leafIDindex+2, 3, forgottennature.logIDindex, 14);    //palm
        }
