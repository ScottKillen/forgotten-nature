...
public class BlockNewLeaves2 extends BlockLeavesBase implements IShearable
{
    ...
    {
        if ((world.getBlockMetadata(i, j, k) & 7) == 5 && !world.isRemote) //if apple on leaf
        {
            world.setBlock(i, j, k, this.blockID, 3, 2);//change to no apple leaf
            dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.appleRed, 1, 0));//drop apple
        }
    }
    	...
    	if (!world.isRemote)
        {
	        ...
	        if ((l & 7) != 0 && world.getBlockLightValue(i, j + 1, k) >= 9 && random.nextInt(4) == 0)
	        {
	        	if(random.nextInt(10) == 0)
	        	{
		            if ((l & 7) == 4)//apple bud
		            {
		                world.setBlock(i, j, k, this.blockID, 5, 2);
		            }
		            if ((l & 7) == 3)//apple leaf
		            {
	            ...
	            if ((l & 7) == 1 && random.nextInt(35) == 0)//cherry
	            {
    ...
    public void dropBlockAsItemWithChance(World world, int i, int j, int k, int l, float f, int i1)
    {
        ...
        if (world.rand.nextInt(14) < 1)
        {
            ...
            {
                dropBlockAsItem_do(world, i, j, k, new ItemStack(forgottennature.newSap, 1, 8));    //sequoia
            }
            if ((l & 7) == 5) //apple & 3,4
            {
                dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.appleRed, 1, 0));
            ...
            if ((l & 7) == 6)
            {
                dropBlockAsItem_do(world, i, j, k, new ItemStack(forgottennature.newSap, 1, 11));    //acacia
            }
            if ((l & 7) == 7)
            {
                dropBlockAsItem_do(world, i, j, k, new ItemStack(forgottennature.newSap, 1, 12));    //Joshua
            }
        }
    }
